...
function EditAdvertising() {
  ...
  console.log('ADV ID',id)


  useEffect(() => {
    if (id != null) {
      axios
        .get(`http://localhost:7070/api/advertising/${id}`)
        .then((res) => {
          const adv = res.data;
          setContent(adv.content)
          setCategory(adv.category)
          setDatePosted(adv.datePosted)
          setStartDate(new Date(adv.endDate).toISOString().split("T")[0]);
          setLocation(adv.location)
          setStartDate(new Date(adv.startDate).toISOString().split("T")[0]);
          setPicture(`http://localhost:7070/${adv.imagePaths}`)
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const res = await axios.put(`http://localhost:7070/api/advertising/${id}`, formData, {
      });
      if(res.status === 200){
